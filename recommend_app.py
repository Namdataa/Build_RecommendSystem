...
st.markdown('<h1 class="blink-text">Hệ thống đề xuất !! </h1>', unsafe_allow_html=True)
...
def recommend(df,jobs,simi):
    movie_index = df[df['Tên công việc'] == jobs].index[0]
    distances = simi[movie_index]
    jobs_list = sorted(list(enumerate(distances)), reverse=True, key=lambda x: x[1])[0:20]
    sorted_jobs_data = df.iloc[[i[0] for i in jobs_list]].head(20)
    return sorted_jobs_data
...
cv = CountVectorizer( max_features=int(len(df)*1.25))
vecto=cv.fit_transform(df['tag']).toarray()
simi=cosine_similarity(vecto)
if key:
    recommend_job = recommend(df,key,simi)
